...
interface SpeisenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Speisen['fields']) => Promise<void>;
  defaultValues?: Speisen['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function SpeisenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: SpeisenDialogProps) {
  const [fields, setFields] = useState<Partial<Speisen['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'speisen');
      await onSubmit(clean as Speisen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "name": string | null, // Name\n  "preis": number | null, // Preis (€)\n  "kategorie": LookupValue | null, // Kategorie (select one key: "speise" | "getraenk") mapping: speise=Speise, getraenk=Getränk\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Speisen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Speisen bearbeiten' : 'Speisen hinzufügen';
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="speise">Speise</SelectItem>
