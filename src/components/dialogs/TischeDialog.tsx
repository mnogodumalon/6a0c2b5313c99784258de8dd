...
interface TischeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Tische['fields']) => Promise<void>;
  defaultValues?: Tische['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function TischeDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: TischeDialogProps) {
  const [fields, setFields] = useState<Partial<Tische['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'tische');
      await onSubmit(clean as Tische['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "tischnummer": string | null, // Tischnummer / Bezeichnung\n  "bemerkung": string | null, // Bemerkung\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Tische['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Tische bearbeiten' : 'Tische hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="tischnummer">Tischnummer / Bezeichnung</Label>
            <Input
              id="tischnummer"
