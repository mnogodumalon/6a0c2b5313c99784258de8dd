...
import { SpeisenDialog } from '@/components/dialogs/SpeisenDialog';
import { SpeisenViewDialog } from '@/components/dialogs/SpeisenViewDialog';
import { TischeDialog } from '@/components/dialogs/TischeDialog';
import { TischeViewDialog } from '@/components/dialogs/TischeViewDialog';
...
const SPEISEN_FIELDS = [
  { key: 'name', label: 'Name', type: 'string/text' },
  { key: 'preis', label: 'Preis (€)', type: 'number' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/radio', options: [{ key: 'speise', label: 'Speise' }, { key: 'getraenk', label: 'Getränk' }] },
];
const TISCHE_FIELDS = [
  { key: 'tischnummer', label: 'Tischnummer / Bezeichnung', type: 'string/text' },
  { key: 'bemerkung', label: 'Bemerkung', type: 'string/text' },
];
const BESTELLUNGEN_FIELDS = [
  { key: 'tisch', label: 'Tisch', type: 'applookup/select', targetEntity: 'tische', targetAppId: 'TISCHE', displayField: 'tischnummer' },
  { key: 'positionen', label: 'Positionen (Speisen)', type: 'applookup/select', targetEntity: 'speisen', targetAppId: 'SPEISEN', displayField: 'name' },
  { key: 'bestellzeit', label: 'Bestellzeit', type: 'date/datetimeminute' },
  { key: 'trinkgeld', label: 'Trinkgeld (€)', type: 'number' },
  { key: 'gesamt', label: 'Gesamtbetrag (€)', type: 'number' },
  { key: 'bezahlt', label: 'Bezahlt', type: 'bool' },
];

const ENTITY_TABS = [
  { key: 'speisen', label: 'Speisen', pascal: 'Speisen' },
  { key: 'tische', label: 'Tische', pascal: 'Tische' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('speisen');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'speisen': return (data as any).speisen as Speisen[] ?? [];
      case 'tische': return (data as any).tische as Tische[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'bestellungen':
        lists.tischeList = (data as any).tische ?? [];
        lists.speisenList = (data as any).speisen ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'bestellungen' && fieldKey === 'tisch') {
      const match = (lists.tischeList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.tischnummer ?? '—';
    }
    if (entity === 'bestellungen' && fieldKey === 'positionen') {
      const match = (lists.speisenList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'speisen': return SPEISEN_FIELDS;
      case 'tische': return TISCHE_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'speisen': return {
        create: (fields: any) => LivingAppsService.createSpeisenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSpeisenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSpeisenEntry(id),
      };
      case 'tische': return {
        create: (fields: any) => LivingAppsService.createTischeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTischeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTischeEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'speisen' || dialogState?.entity === 'speisen') && (
        <SpeisenDialog
          open={createEntity === 'speisen' || dialogState?.entity === 'speisen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'speisen' ? handleUpdate : (fields: any) => handleCreate('speisen', fields)}
          defaultValues={dialogState?.entity === 'speisen' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Speisen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Speisen']}
        />
      )}
      {(createEntity === 'tische' || dialogState?.entity === 'tische') && (
        <TischeDialog
          open={createEntity === 'tische' || dialogState?.entity === 'tische'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'tische' ? handleUpdate : (fields: any) => handleCreate('tische', fields)}
          defaultValues={dialogState?.entity === 'tische' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'bestellungen' || dialogState?.entity === 'bestellungen') && (
        <BestellungenDialog
          ...
          tischeList={(data as any).tische ?? []}
          speisenList={(data as any).speisen ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Bestellungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Bestellungen']}
        />
      )}
      {viewState?.entity === 'speisen' && (
        <SpeisenViewDialog
          open={viewState?.entity === 'speisen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'speisen', record: r }); }}
        />
      )}
      {viewState?.entity === 'tische' && (
        <TischeViewDialog
          open={viewState?.entity === 'tische'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'tische', record: r }); }}
        />
      )}
