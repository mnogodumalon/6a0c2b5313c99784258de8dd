...
import type { Speisen } from '@/types/app';
...
export default function SpeisenPage() {
  const [records, setRecords] = useState<Speisen[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Speisen | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Speisen | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Speisen | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getSpeisen());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Speisen['fields']) {
    await LivingAppsService.createSpeisenEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Speisen['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateSpeisenEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteSpeisenEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Speisen"
      subtitle={`${records.length} Speisen im System`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Speisen. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
