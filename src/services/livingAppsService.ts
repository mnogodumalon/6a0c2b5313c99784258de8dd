...
import type { Speisen, Tische, Bestellungen, CreateSpeisen, CreateTische, CreateBestellungen } from '@/types/app';
...
export class LivingAppsService {
  // --- SPEISEN ---
  static async getSpeisen(): Promise<Speisen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SPEISEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Speisen[];
    return enrichLookupFields(records, 'speisen');
  }
  static async getSpeisenEntry(id: string): Promise<Speisen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SPEISEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Speisen;
    return enrichLookupFields([record], 'speisen')[0];
  }
  static async createSpeisenEntry(fields: CreateSpeisen) {
    return callApi('POST', `/apps/${APP_IDS.SPEISEN}/records`, { fields: cleanFieldsForApi(fields as any, 'speisen') });
  }
  static async updateSpeisenEntry(id: string, fields: Partial<CreateSpeisen>) {
    return callApi('PATCH', `/apps/${APP_IDS.SPEISEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'speisen') });
  }
  static async deleteSpeisenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SPEISEN}/records/${id}`);
  }

  // --- TISCHE ---
  static async getTische(): Promise<Tische[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TISCHE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Tische[];
    return enrichLookupFields(records, 'tische');
  }
  static async getTischeEntry(id: string): Promise<Tische | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TISCHE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Tische;
    return enrichLookupFields([record], 'tische')[0];
  }
  static async createTischeEntry(fields: CreateTische) {
    return callApi('POST', `/apps/${APP_IDS.TISCHE}/records`, { fields: cleanFieldsForApi(fields as any, 'tische') });
  }
  static async updateTischeEntry(id: string, fields: Partial<CreateTische>) {
    return callApi('PATCH', `/apps/${APP_IDS.TISCHE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'tische') });
  }
  static async deleteTischeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TISCHE}/records/${id}`);
  }
